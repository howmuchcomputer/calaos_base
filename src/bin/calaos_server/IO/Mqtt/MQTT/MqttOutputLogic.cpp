...
void _connect_received_output_cb(void *data, EMqtt_Sn_Client *client, EMqtt_Sn_CONNECTION_STATE connection_state);
void _puback_received_output_cb(void *data, EMqtt_Sn_Client *client);
    ...
    port(1883)
{
    ...
    client = MqttMap::Instance(address, port, _connect_received_output_cb, (void*)this, 10.0);
...
bool MqttOutputLogic::set_value_real(bool val)
{
    ...
    if(val == true){
	printf("<<<<<<<<<<<<<<<<<<< YELLOW\n");
	emqtt_sn_client_send_publish(client, "led", "yellow", _puback_received_output_cb, NULL);
    }else{
	printf("<<<<<<<<<<<<<<<<<<< RED\n");
	emqtt_sn_client_send_publish(client, "led", "red", _puback_received_output_cb, NULL);
    }
    return true;
...
void _puback_received_output_cb(void *data, EMqtt_Sn_Client *client)
{
    printf("PUBACK RECEIVED for Output Logic\n\n");
}
